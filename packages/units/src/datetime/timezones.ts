...
export const getTimeZones = memoize(
  (includeInternal: boolean | InternalTimeZones[] = false): TimeZone[] => {
...
export const getTimeZoneGroups = memoize(
  (
    includeInternal: boolean | InternalTimeZones[] = false,
  ): GroupedTimeZones[] => {
    const timeZones = getTimeZones(includeInternal);

    const groups = timeZones.reduce(
      (groups: Record<string, TimeZone[]>, zone: TimeZone) => {
...
const countriesByTimeZone = ((): Record<string, TimeZoneCountry[]> => {
  return moment.tz
    .countries()
    .reduce((all: Record<string, TimeZoneCountry[]>, code) => {
      const timeZones = moment.tz.zonesForCountry(code);
      return timeZones.reduce(
        (all: Record<string, TimeZoneCountry[]>, timeZone) => {
          if (!all[timeZone]) {
            all[timeZone] = [];
          }
