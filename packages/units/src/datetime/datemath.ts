...
export function isMathString(text: string | DateTime | Date): boolean {
  ...
  if (
    typeof text === "string" &&
    (text.substring(0, 3) === "now" || text.includes("||"))
  ) {
    return true;
  } else {
    return false;
...
export function toDateTime(
  dateTimeRep: string | DateTime | Date,
...
export function roundToFiscal(
  ...
  switch (unit) {
    case "y":
      if (roundUp) {
        roundToFiscal(fyStartMonth, dateTime, unit, false)
          ?.add(11, "M")
          .endOf("M");
      } else {
        dateTime
          .subtract(
            (dateTimeAsMoment(dateTime).month() - fyStartMonth + 12) % 12,
            "M",
          )
          .startOf("M");
      }
    ...
    case "Q":
      if (roundUp) {
        ...
        dateTime
          .subtract(
            (dateTimeAsMoment(dateTime).month() - fyStartMonth + 12) % 3,
            "M",
          )
          .startOf("M");
      }
