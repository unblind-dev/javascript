 ...
 * @public
 */
export function intervalToAbbreviatedDurationString(
  interval: Interval,
  includeSeconds = true,
): string {
...
export function reverseParseDuration(
  duration: Duration,
  ...
  return Object.entries(duration).reduce((str, [unit, value]) => {
    if (
      value &&
      value !== 0 &&
      !(unit === "seconds" && !includeSeconds && str)
    ) {
      const padding = str !== "" ? " " : "";
      return str + `${padding}${value}${(durationMap[unit] as any)[0]}`;
    }
...
export function parseDuration(durationString: string): Duration {
  return durationString.split(" ").reduce<Duration>((acc, value) => {
    ...
    const mapping = Object.entries(durationMap).find(([_, abbreviations]) =>
      abbreviations?.includes(match[2] as string),
    );
...
export function isValidDuration(durationString: string): boolean {
  for (const value of durationString.trim().split(" ")) {
    ...
    const key = Object.entries(durationMap).find(([_, abbreviations]) =>
      abbreviations?.includes(match[2] as string),
    )?.[0];
    if (!key) {
      return false;
...
function validateDurationByUnits(
  ...
  for (const value of durationString.trim().split(" ")) {
    ...
    const isValidUnit = timeUnits.includes(match[2] as string);
    if (!isValidUnit) {
      return false;
    }
  }
