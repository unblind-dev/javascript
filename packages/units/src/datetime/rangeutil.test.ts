import { initRegionalFormat as initRegionalFormatForTests } from "../i18n/dates";
...
describe("Range Utils", () => {
  ...
  describe("convertRawToRange", () => {
    ...
    it("should serialize using custom formats", () => {
      ...
      const deserializedTimeRange = convertRawToRange(
        nonDefaultRawTimeRange,
        undefined,
        undefined,
        NON_DEFAULT_FORMAT,
      );
      expect(deserializedTimeRange.from.format()).toBe(
        DEFAULT_DATE_VALUE_FORMATTED,
      );
    });
  ...
  describe("relativeToTimeRange", () => {
    it("should convert seconds to timeRange", () => {
      ...
      const timeRange = relativeToTimeRange(
        relativeTimeRange,
        dateTime("2021-04-20T15:55:00Z"),
      );

      expect(timeRange.from.valueOf()).toEqual(
        dateTime("2021-04-20T15:45:00Z").valueOf(),
      );
      expect(timeRange.to.valueOf()).toEqual(
        dateTime("2021-04-20T15:50:00Z").valueOf(),
      );
    });
    ...
    it("should convert from now", () => {
      ...
      expect(timeRange.to.valueOf()).toEqual(
        dateTime("2021-04-20T15:55:00Z").valueOf(),
      );
    });
  });
  ...
  describe("describeTimeRange", () => {
    it.each([
      ...
      'should return display name "%s" for range from "%s" to "%s"',
      (from, to, expected) => {
    ...
    it("should prioritize custom quick ranges over standard ranges", () => {
      const customRanges = [
        { from: "now-5m", to: "now", display: "Lightning round!" },
      ];
    ...
    it("should handle empty custom ranges array", () => {
      expect(
        describeTimeRange({ from: "now-5m", to: "now" }, undefined, []),
      ).toBe("Last 5 minutes");
    });

    it("should handle null custom ranges", () => {
      expect(
        describeTimeRange({ from: "now-5m", to: "now" }, undefined, undefined),
      ).toBe("Last 5 minutes");
    });
  });
  ...
  describe("describeTimeRange - localeFormatPreference enabled", () => {
    ...
    beforeAll(() => {
      ...
      mockGetFeatureToggle = jest
        .spyOn(featureToggles, "getFeatureToggle")
        .mockImplementation((featureName) => {
